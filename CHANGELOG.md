...
- `phase3`: Legacy `keycloak-operator` CRD pin bumped from `19.0.2` to `19.0.3` (last stable release in the `keycloak.org/v1alpha1` lineage). The full Keycloak 19→26 migration is **deferred** to a follow-up phase because it requires a near-rewrite of the role and cross-role refactors of every `KeycloakClient` / `KeycloakUser` template (the new `k8s.keycloak.org/v2alpha1` operator removed those CRDs in favor of a single `KeycloakRealmImport` resource). See `roles/keycloak/MIGRATION-26.md` for the plan.
